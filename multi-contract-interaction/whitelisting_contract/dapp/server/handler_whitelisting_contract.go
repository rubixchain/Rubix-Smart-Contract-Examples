...
type SmartContractDataInput struct {
	Did string `json:"did"`
}
...
func executeWASMContract(wasmDir string, wasmFuncName string, states []SCTDataReply) ([]string, error) {
	...
	for idx, state := range states {
		...
		wasmOutput, err := wasmModule.CallWasmFunc(wasmFuncName, smartContractDataInput)
		if err != nil {
			return nil, fmt.Errorf("err occured while calling wasm func %v for input %v, err: %v", wasmFuncName, smartContractDataInput, err)
		}
...
func runWhitelistContractHandle(w http.ResponseWriter, r *http.Request) {
	...
	whitelistContractFunc := "white_list_did"
	wasmFileDir := os.Getenv("WASM_FILE_DIR")

	outputs, err := executeWASMContract(wasmFileDir, whitelistContractFunc, latestContractData)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func syncStateHandler(w http.ResponseWriter, _ *http.Request) {
	...
	whitelistedDIDs, err := executeWASMContract(wasmFileDir, whitelistContractFunc, states)
	if err != nil {
		fmt.Println(err)
		return
	}
